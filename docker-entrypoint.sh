...
# GitHub, builds it, and runs the four production processes (web, formular
# API, chat API, vector API). To deploy a new version: restart/recreate this container.
#
...
echo "==> Starting vector-server.mjs (port 8092)"
...
cleanup() {
  kill "$FORMULAR_PID" "$CHAT_PID" "$VECTOR_PID" "$WEB_PID" 2>/dev/null || true
}
trap cleanup EXIT INT TERM

# If any of the four processes dies, exit so Docker's restart policy
# recreates the container — which also picks up any newer commit on next start.
wait -n "$FORMULAR_PID" "$CHAT_PID" "$VECTOR_PID" "$WEB_PID"
